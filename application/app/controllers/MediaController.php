			...
			if ($validation->passes() && $valid_media)
			{
				...
				if(isset($input['pic'])){
					if(isset($input['img_url']) && $input['img_url'] != ''){
						$input['pic_url'] = Helper::uploadImage($input['img_url'], 'images', Helper::slugify($input['title']), 'url');
					} else if(isset($input['pic_url'])){

						$input['pic_url'] = Helper::uploadImage(Input::file('pic_url'), 'images');
					}
					$input['pic'] = 1;
				}
	...
	public function delete($id)
	{
		...
		if($media->user_id == Auth::user()->id || Auth::user()->admin == 1){
			...
			if(strpos($media->pic_url, '.gif') > 0){
				if(file_exists('./content/uploads/images/' . str_replace(".gif", "-animation.gif", $media->pic_url))){
					unlink('./content/uploads/images/' . str_replace(".gif", "-animation.gif", $media->pic_url));
				}
			}

			// remove the image
			if(file_exists('./content/uploads/images/' . $media->pic_url)){
				unlink('./content/uploads/images/' . $media->pic_url);
			}
